...
import com.coderpig.drysister.DryConstant;
import com.coderpig.drysister.R;
import com.coderpig.drysister.ui.fragment.LittleSisterFragment;
import com.coderpig.drysister.ui.fragment.NewsFragment;
import com.coderpig.drysister.ui.fragment.SubwayFragment;
import com.coderpig.drysister.ui.fragment.ToolsFragment;
import com.coderpig.drysister.ui.fragment.WeatherFragment;
import com.coderpig.drysister.utils.PackageUtils;
import com.coderpig.drysister.utils.ResUtils;
...
public class MainActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    private void initData() {
        mFgManager.beginTransaction().replace(R.id.cly_main_content, LittleSisterFragment.newInstance(), DryConstant.FG_LITTLE_SISTER).commit();
        toolbar.setTitle(ResUtils.getString(R.string.menu_see_little_sister));
        String version = PackageUtils.packageVersionNName();
        if(version != null) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.nav_see_news:
                if (mFgManager.findFragmentByTag(DryConstant.FG_NEWS) == null) {
                    mFgManager.beginTransaction().replace(R.id.cly_main_content, NewsFragment.newInstance(), DryConstant.FG_NEWS).commit();
            ...
            case R.id.nav_use_check_weather:
                if (mFgManager.findFragmentByTag(DryConstant.FG_WEATHER) == null) {
                    mFgManager.beginTransaction().replace(R.id.cly_main_content, WeatherFragment.newInstance(), DryConstant.FG_WEATHER).commit();
            ...
            case R.id.nav_use_check_subway:
                if (mFgManager.findFragmentByTag(DryConstant.FG_SUBWAY) == null) {
                    mFgManager.beginTransaction().replace(R.id.cly_main_content, SubwayFragment.newInstance(), DryConstant.FG_SUBWAY).commit();
            ...
            case R.id.nav_use_tools:
                if (mFgManager.findFragmentByTag(DryConstant.FG_TOOLS) == null) {
                    mFgManager.beginTransaction().replace(R.id.cly_main_content, ToolsFragment.newInstance(), DryConstant.FG_TOOLS).commit();
